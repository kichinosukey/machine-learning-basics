...
def test__gradientDescent():
    ...
    theta_hist, j_hist = gradientDescent(X, y, theta, alpha, iterations, history=True)
    ...
    theta_min, j = gradientDescent(X, y, theta, 0.01, 10)
    np.testing.assert_allclose(theta_min, np.array([[0.1855552 ], [0.50436048], [0.40137032]]), rtol=1e-06)

    theta_hist, j_hist = gradientDescent(X, y, theta, 0.01, 10, history=True)
    np.testing.assert_allclose(j_hist, 
